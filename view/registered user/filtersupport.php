...
function getMyDiscussionAnswers($discussion_id){
  global $conn;
  global $user_id;
  $sql = "SELECT * FROM answer WHERE discussion_id=$discussion_id AND user_id=$user_id ORDER BY priority_flag DESC, timestamp DESC";
  $result = mysqli_query($conn, $sql);
  return $result;
}
